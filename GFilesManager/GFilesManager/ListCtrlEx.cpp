...
BOOL CListCtrlEx::OnNMDblclk(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	int CLRow = pNMListView->iItem;
	int CLColumn = pNMListView->iSubItem;
...
#endif
{
	if (nIDEvent == CONST_TIMER_ID && m_pDragImage)
	{
		WPARAM wParam = NULL;
		int nDropIndex = -1;
...
void CListCtrlEx::SetScrollTimer(EScrollDirection ScrollDirection)
{
	if (m_uScrollTimer == 0)
	{
		m_uScrollTimer = SetTimer(CONST_TIMER_ID, 100, NULL);
	}
