...
BOOL CListCtrlEx::OnNMDblclk(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	RowEditList = pNMListView->iItem;
	ColumnEditList = pNMListView->iSubItem;
	
	*pResult = DisplayEditor(RowEditList, ColumnEditList);
...
void CListCtrlEx::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (nItem >= 0)
	{
		...
		if (nSubitem == 0 && szval != L"") ::SetCursor(AfxGetApp()->LoadStandardCursor(IDC_HAND));
