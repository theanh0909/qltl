...
BOOL CRegistry::WriteChar(char* strName, char* strValue)
{
	ASSERT(m_strCurrentPath.GetLength() > 0);
	BOOL bSuccess = TRUE;
	HKEY hKey;
	...
	sprintf(tmp, "%s", strValue);


	if (::RegOpenKeyEx(m_hRootKey, LPCTSTR(m_strCurrentPath), 0,
		KEY_WRITE, &hKey) != ERROR_SUCCESS) return FALSE;
	LONG lResult;
	lResult = RegSetValueExA(hKey, strName , 0, REG_SZ, (BYTE*) tmp, strlen(tmp)+1);
	if (lResult != ERROR_SUCCESS)
		lResult = RegSetValueExA(hKey, strName, 0, REG_SZ, NULL, 0);
