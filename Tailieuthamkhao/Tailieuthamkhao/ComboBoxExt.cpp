...
BOOL CComboBoxExt::OnEditchange()
{
	...
	sEditText.MakeLower();
	sEditText = ff._ConvertKhongDau(sEditText);

	m_bEdit = TRUE;
	int _pos = -1, _ichk = 0, _ifail = 0;
	CString sTemp = L"", sFirstOccurrence = L"";
	POSITION pos = m_PtrList.GetHeadPosition();
	while (!m_sTypedText.IsEmpty() && pos)
	{
		...
		sTemp = pData->m_sItem;
		sTemp.MakeLower();
		sTemp = ff._ConvertKhongDau(sTemp);

		_ifail = 0;
		vector<CString> vecLine;
		ff._TackTukhoa(vecLine, sEditText, L" ", L"+", 0, 0);
		for (int i = 0; i < (int)vecLine.size(); i++)
		{
			_pos = sTemp.Find(vecLine[i]);
			if (_pos == -1)
			{
				_ifail = 1;
				break;
		...
		if (_ifail == 0 && (int)vecLine.size() > 0)
		{
			if (_pos == 0 && _ichk == 0) _ichk = 1;
			AddItem(pData);
		}
		else DeleteItem(pData);
	...
	if (GetCount() < 1 || m_sTypedText.IsEmpty())
	{
		...
		else
		{
			...
			while (NULL != pos) AddItem(m_PtrList.GetNext(pos));
		}
	}
	else
	{
		ShowDropDown();
		FitDropDownToItems();
		if (MODE_AUTOCOMPLETE == m_nMode && GetCount() > 0) GetLBText(0, sFirstOccurrence);
	}

	CString szval = L"";
	if (_ichk == 1)
	{
		szval = sFirstOccurrence;
		szval.MakeLower();
		szval = ff._ConvertKhongDau(szval);

		if (szval.Left(sEditText.GetLength()) == sEditText)
		{
...
// Error	173	error C2440: 'static_cast' : cannot convert from 'void (__cdecl CComboBoxExt::* )(UINT)' to 'void (__cdecl CWnd::* )(UINT_PTR)'	D:\CODE\QLCL2015\QLCL2015\ComboBoxExt.cpp	98
// --> Chuyển từ OnTimer(UINT nIDEvent) sang OnTimer(UINT_PTR nIDEvent)
